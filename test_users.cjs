...
async function check() {
  ...
  m.forEach(doc => {
    const data = doc.data();
    if (data.type === 'admin' || data.type === 'superuser' || data.name.toLowerCase().includes('admin')) {
      console.log(`Admin/Superuser: ${data.name}, Type: ${data.type}, Passcode: ${data.passcode}`);
    }
  });
  process.exit(0);
}
